...
NetworkUtilsServer::NetworkUtilsServer(unsigned short port) : listen_port(port)
{
...
void NetworkUtilsServer::ConnectClients(std::vector<sf::TcpSocket *> *client_array)
{
    while (true)
    {
...
void NetworkUtilsServer::DisconnectClient(sf::TcpSocket *socket_pointer, size_t position)
{
...
void NetworkUtilsServer::BroadcastPacket(sf::Packet &packet, sf::IpAddress exclude_address, unsigned short port)
{
...
void NetworkUtilsServer::ReceivePacket(sf::TcpSocket *client, size_t iterator)
{
    sf::Packet packet;
...
void NetworkUtilsServer::ManagePackets()
{
    while (true)
    {
        for (size_t iterator = 0; iterator < client_array.size(); iterator++)
...
void NetworkUtilsServer::Run()
{
    std::thread connetion_thread(&NetworkUtilsServer::ConnectClients, this, &client_array);
