...
void ServerNetwork::CheckMessType(sf::Packet &packet, sf::IpAddress exclude_address, unsigned short port)
{
    logl("p2");
    sf::Uint8 mType;
    packet >> mType;
    ...
    case 1:
        logl("p2.5");
        ServerNetwork::SignInMessPac(packet, exclude_address, port);
        break;
...
void ServerNetwork::SignInMessPac(sf::Packet &packet, sf::IpAddress exclude_address, unsigned short port)
{
    log("p3");
    std::string username = "thai";
    std::string password = "thai";

    SignInMess signInMess;
    packet >> signInMess;
    packet.clear();
    logl(signInMess.username);
    if (signInMess.username == username && signInMess.password == password)
    {
        logl("Hello");
        // sf::Uint8 sendPac = 1;
        // packet << sendPac;
...
void ServerNetwork::ReceivePacket(sf::TcpSocket *client, size_t iterator)
{
    ...
    {
        logl("p1");
        CheckMessType(packet, client->getRemoteAddress(), client->getRemotePort());
        // packet.clear();
